...
from real_world_imitation.utils.general_utils import ConcatSequential, HasParameters
...
class Block(nn.Sequential, HasParameters):
    ...
    def get_default_params(self):
        params = AttrDict(
            ...
            dropout_rate=self.builder.dropout_rate
        )
        return params
    ...
    def complete_block(self):
        if self.params.dropout:
            self.add_module('dropout', nn.Dropout(self.params.dropout_rate))
...
class BaseProcessingNet(ConcatSequential):
    ...
    def __init__(self, in_dim, mid_dim, out_dim, num_layers, builder, block=None, detached=False,
                 final_activation=None, initial_batchnorm=False):
        ...
        ### Batchnorm input as per R3M
        if initial_batchnorm:
            self.add_module('first_norm', nn.BatchNorm1d(in_dim, **AttrDict(affine=True, track_running_stats=True)))
        ###
        self.add_module('input', block(in_dim=in_dim, out_dim=mid_dim, normalization=builder.normalization, dropout=builder.dropout)) #earlier: normalization=None/builder.normalization
        for i in range(num_layers):
            self.add_module('pyramid-{}'.format(i),
                            block(in_dim=mid_dim, out_dim=mid_dim, normalize=builder.normalize, dropout=builder.dropout))

        self.add_module('head'.format(i + 1),
                        block(in_dim=mid_dim, out_dim=out_dim, normalization=None, activation=final_activation, dropout=False))
...
class LayerBuilderParams:
    ...
    def __init__(self, use_convs, normalization='batch', dropout_rate=None):
        self.use_convs = use_convs
        self.init_fn = init_weights_xavier
        self.normalize = normalization != 'none'
        self.normalization = normalization
        self.dropout = (dropout_rate != None) and (dropout_rate != 0)
    ...
    def wrap_block(self, block):
        ...
        return partial(block, builder=self, normalization=self.normalization, dropout_rate=self.dropout_rate)
