...
class GMM:
    ...
    def log_prob(self, x):
        return torch.logsumexp(torch.log(self.pi) +
                               MultivariateGaussian(self.mu, self.log_sigma).log_prob(x.unsqueeze(-2)), dim=-1)
    ...
    def sample_greedy(self):
        """Sample the highest probability point, i.e. the mean of the a gaussian with max log_prob"""
        probs = torch.stack([self.log_prob(mu) for mu in self.mu], dim=0)
        return self.mu[torch.argmax(probs)]
    ...
    def kl_divergence(self, other):
        assert len(self._components) == len(other._components)
        return
        return torch.tensor([10,10], dtype=torch.float32)
        kl_min = None
        for components in permutations(self._components, len(self._components)):
            kl_curr = torch.stack([p.kl_divergence(q) for p, q in zip(components, other._components)])
            
            if kl_min==None or kl_curr.mean() < kl_min.mean():
                kl_min = kl_curr
        return kl_min
...
if __name__ == "__main__":
    ...
    from real_world_imitation.utils.general_utils import split_along_axis
    # import numpy as np
    ...
    def _draw_gaussian(ax, gauss_tensor, color, weight=None):
        ...
        ell.set_alpha(float(weight))
        # print(weight)
        # if False and weight is not None:
        #     ell.set_alpha(int(weight))
        # else:
    ...
    # ax = plt.subplot(111)
    # plt.xlim(-2, 2); plt.ylim(-2, 2)
    # [_draw_gaussian(ax, component.tensor(), 'green', ten2ar(weight)) for weight, component in data_dist[0]]
    
    # samples = np.concatenate([data_dist.sample() for _ in range(1)])
    # plt.scatter(samples[:, 0], samples[:, 1])
    # plt.show()
    # exit(0)
