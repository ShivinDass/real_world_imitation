...
from torch.nn.modules import BatchNorm1d, BatchNorm2d, BatchNorm3d, Dropout
from torch.nn.functional import interpolate

from real_world_imitation.utils.general_utils import batchwise_assign, map_dict, AttrDict, AverageMeter, map_recursive, remove_spatial
from real_world_imitation.utils import ndim
...
def switch_off_dropout_update(model):
    """Switches off batchnorm update in all submodules of model."""
    for module in model.modules():
...
def switch_on_dropout_update(model):
    """Switches on batchnorm update in all submodules of model."""
    for module in model.modules():
        if isinstance(module, Dropout):
            module.train()
...
@contextmanager
def no_dropout_update(model):
    switch_off_dropout_update(model)
    yield
    switch_on_dropout_update(model)
