...
class InterProcessCommunication:
    # Simple inter process communication between docker and pc through shared file writing
    def __init__(self, file_path, unique_id):

        self.file_path = file_path
        self.unique_id = unique_id
        if not os.path.exists(self.file_path):
            print("Error: {} not a valid file path".format(self.file_path))
            exit(0)
        print("==> Communication on file {}".format(self.file_path))

    def send_message(self, data):
        with open(self.file_path, 'w') as f:
            f.writelines("{}:{}".format(self.unique_id, data))
    ...
    def read_message(self):
        with open(self.file_path, 'r') as f:
            ...
            if not line:
                return ""
            id, message = line[0].split(":")
        if not id == self.unique_id:
            return message
        return ""