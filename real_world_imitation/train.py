...
from real_world_imitation.components.data_loader import RandomVideoDataset
from real_world_imitation.utils.general_utils import RecursiveAverageMeter, map_dict
from real_world_imitation.components.checkpointer import CheckpointHandler, save_cmd, save_git, get_config_path
from real_world_imitation.utils.general_utils import dummy_context, AttrDict, get_clipped_optimizer, \
                                                        AverageMeter, ParamDict
from real_world_imitation.utils.pytorch_utils import LossSpikeHook, NanGradHook, NoneGradHook, \
                                                        DataParallelWrapper, RAdam
from real_world_imitation.components.trainer_base import BaseTrainer
from real_world_imitation.utils.wandb import WandBLogger
from real_world_imitation.components.params import get_args
...
class ModelTrainer(BaseTrainer):
    def __init__(self, args):
        ...
        self.optimizer = self.get_optimizer_class()(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self._hp.lr, weight_decay=self._hp.l2_penalty)
        self.schedular = ExponentialLR(self.optimizer, self._hp.lr_decay) if (self._hp.lr_decay != 1) else None
    ...
    def _default_hparams(self):
        default_dict = ParamDict({
            ...
            'grad_clip': None,  #Gradient clipping overall
    ...
    def train(self, start_epoch):
        ...
        for epoch in range(start_epoch, self._hp.num_epochs):
            ...
            if (not self.args.dont_save) and (epoch%5==0 or epoch > 5):
                save_checkpoint({
                    'epoch': epoch,
            ...
            if self.schedular and epoch < self._hp.lr_decay_steps:
                self.schedular.step()
    ...
    def train_epoch(self, epoch):
        ...
        for self.batch_idx, sample_batched in enumerate(self.train_loader):
            ...
            with self.training_context():
                ...
                elif self._hp.grad_clip:
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), self._hp.grad_clip)
                self.optimizer.step()
                self.model.step()
